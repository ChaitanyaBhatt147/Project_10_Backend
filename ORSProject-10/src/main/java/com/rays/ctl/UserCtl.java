...
public class UserCtl extends BaseCtl<UserForm, UserDTO, UserServiceInt> {
	...
	/*
	 * @PostMapping("changePassword") public ORSResponse
	 * changePassword(@RequestBody @Valid ChangePasswordForm form, BindingResult
	 * bindingResult) {
	 * 
	 * ORSResponse res = validate(bindingResult);
	 * 
	 ...
	 * 
	 * UserDTO changedDto = baseService.changePassword(form.getLoginId(),
	 * form.getOldPassword(), form.getNewPassword(), userContext);
	 * 
	 * if (changedDto == null) { res.setSuccess(false);
	 * res.addMessage("Invalid old password"); return res; }
	 * 
	 * res.setSuccess(true); res.addMessage("Password has been changed");
	 * 
	 * return res; }
	 */
}